...
function Shortener({ loginData, setLoginData, apiKey, setApiKey }) {
  ...
  useEffect(() => {
    ...
    //   process.env.NODE_ENV === 'production'
        `https://scissors-api-ew6j.onrender.com/api/urls/?user=${loginData.email}`
  ...
  const handleShortenWithKey = () => {
    ...
    if (url.match(regex)) {
      let api_url = `https://scissors-api-ew6j.onrender.com/api/urls/encrypt/`;
  ...
  const handleShorten = () => {
    ...
    if (url.match(regex)) {
      let api_url = `https://scissors-api-ew6j.onrender.com/api/urls/`;
